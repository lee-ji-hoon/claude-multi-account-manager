...
```bash
# 1. 저장소 클론
git clone https://github.com/lee-ji-hoon/claude-multi-account-manager.git

# 2. 설치 스크립트 실행
cd claude-multi-account-manager
./install.sh
```

설치 후 Claude Code를 재시작하면 `/account` 명령어 사용 가능!
...
# 2. 플러그인 디렉토리에 복사
mkdir -p ~/.claude/plugins/cache/local/account-manager/1.0.0
cp -r claude-multi-account-manager/{.claude-plugin,commands,hooks,hooks-handlers,account_manager.py} \
  ~/.claude/plugins/cache/local/account-manager/1.0.0/
...
data['plugins']['account-manager@local'] = [{
    'scope': 'user',
    'installPath': os.path.expanduser('~/.claude/plugins/cache/local/account-manager/1.0.0'),
    'version': '1.0.0',
...
```bash
# 클론한 디렉토리에서 직접 실행 (일회성)
claude --plugin-dir ./claude-multi-account-manager
```
