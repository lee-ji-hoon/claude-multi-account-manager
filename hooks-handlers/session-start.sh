...
write_v2_block() {
    local rc="$1"
    {
        ...
        echo "# account-switch: long-lived 토큰일 때 CLAUDE_CODE_OAUTH_TOKEN env 자동 export"
        echo "account-switch() {"
        echo "    local output"
        echo "    output=\$(_account_mgr_run switch --shell-export \"\$@\")"
        echo "    local rc=\$?"
        ...
        echo "    return \$rc"
        echo "}"
        echo "# account-export-token: 토큰을 자동 env export"
        echo "account-export-token() {"
        echo "    local output"
        echo "    output=\$(_account_mgr_run export-token \"\$@\")"
        echo "    local rc=\$?"
        echo "    while IFS= read -r line; do"
        echo "        case \"\$line\" in"
        echo "            'export CLAUDE_CODE_OAUTH_TOKEN='*)"
        echo "                eval \"\$line\""
        echo "                echo \"  CLAUDE_CODE_OAUTH_TOKEN 환경변수 설정됨\""
        echo "                ;;"
        echo "            *)"
        echo "                printf '%s\\n' \"\$line\""
        echo "                ;;"
