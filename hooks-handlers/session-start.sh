...
BLOCK_VERSION_TAG="# account-manager-block: v4"
...
write_v2_block() {
    local rc="$1"
    {
        ...
        echo "# 이전 정의 정리 (v3 마이그레이션 등)"
        echo "unset -f account-switch account-export-token 2>/dev/null"
        echo "unalias account account-switch account-list account-export-token 2>/dev/null"
        ...
        echo "    local subcmd=\"\$1\""
        echo "    case \"\$subcmd\" in"
        echo "        switch|export-token)"
        echo "            # CLAUDE_CODE_OAUTH_TOKEN env 자동 동기화"
        echo "            shift"
        echo "            local output"
        echo "            output=\$(python3 \"\$base/\$latest/account_manager.py\" \"\$subcmd\" --shell-export \"\$@\")"
        echo "            local rc=\$?"
        echo "            while IFS= read -r line; do"
        echo "                case \"\$line\" in"
        echo "                    'export CLAUDE_CODE_OAUTH_TOKEN='*|'unset CLAUDE_CODE_OAUTH_TOKEN'*)"
        echo "                        eval \"\$line\""
        echo "                        ;;"
        echo "                    *)"
        echo "                        printf '%s\\n' \"\$line\""
        ...
        echo "alias account-export-token='_account_mgr_run export-token'"
        echo "$MARKER_END"
    } >> "$rc"
}
